...
class NewtonianSimulation(DrivingSimulation):
    ...
    def step(self):
        obj_positions = []
        for obj in self.objects:
            reverse_flag = -1 if obj.reverse else 1
            if obj.hand_brake:
                t_action = -1.0
                acceleration = -MAX_BRAKING
                obj.speed += acceleration * self.timestep
                ...
                acceleration = obj.throttle * MAX_ACCELERATION * reverse_flag
                obj.speed += acceleration * self.timestep
            obj.velocity = Vector(0, obj.speed).rotatedBy(obj.heading)
            if obj.steer:
                s_action = obj.steer
            ...
            obj_positions.append(tuple([obj.position.x, obj.position.y, obj.heading]))
        self.traj.append(tuple(obj_positions))
        if self.render:
            self.draw_objects()
